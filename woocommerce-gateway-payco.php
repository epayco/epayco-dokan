 ...
 *
 * @wordpress-plugin
 * Plugin Name:       ePayco Gateway WooCommerce and Dokan
 * Description:       Plugin ePayco Gateway for WooCommerce and Dokan.
 * Version:           1.0.0
 * Author:            ePayco
...
if (in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) {
    ...
    function init_epayco_woocommerce()
    {
        ...
        class WC_ePayco extends WC_Payment_Gateway
        {
            ...
            public function receipt_page($order_id)
            {
                ...
                foreach ($order->get_items() as $product => $item ) {
                    $epayco_p_cust_id_client = get_post_meta( $item["product_id"], 'p_cust_id_client' );
                    $vendor_id = get_post_field( 'post_author', $item["product_id"] );
                    $vendor = get_userdata( $vendor_id );
                    $productQuantity = $item["quantity"];
                    $table_name_ = $wpdb->prefix . "usermeta";
                    $sql = 'SELECT meta_value FROM '.$table_name_ .' WHERE user_id = '. intval($vendor_id).' AND meta_key IN (
                        "dokan_enable_selling","dokan_admin_percentage","dokan_admin_percentage_type")';
                    $results = $wpdb->get_results($sql, OBJECT);                    $fee = floatval($item['total']);
                    $fee = floatval($item['total']);
                    if($results){
                        if($results[2]->meta_value == "yes"){
                            $typeFee = $results[1]->meta_value;
                            if($typeFee == "flat"){
                                $fee = floatval($results[0]->meta_value)*$productQuantity;
                            }else{
                                $valueFeePercent = (floatval($item['total'])* floatval($results[0]->meta_value))/100;
                                $fee = $valueFeePercent;
                            }
                            $receivers['id'] = $epayco_p_cust_id_client[0];
                            $receivers['total'] = floatval($item['total'])+floatval($item['total_tax']) ;
                            $receivers['iva'] = floatval($item['total_tax']);
                            $receivers['base_iva'] = floatval($item['total']);
                            $receivers['fee'] = $fee;
                        }
                    }

                    $clearData = str_replace('_', ' ', $this->string_sanitize($item['name']));
    ...
    function epayco_product_panels(){
        ...
        woocommerce_wp_text_input( array(
            ...
            'label'             => 'Id customer',
            'description'       => 'Id del usuario que va a recibir comisión'
        ) );
    }
